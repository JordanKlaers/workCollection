...
export default {
	...
	mounted() {
		const canvasContainer = document.querySelector('#notification-card .frame').getBoundingClientRect();
		const navContainer = document.querySelector('#notification-card .nav-container').getBoundingClientRect();
		const displayCard = document.querySelector('#notification-card .display-card').getBoundingClientRect();
		this.canvasWidth = canvasContainer.width;
		this.canvasHeight = canvasContainer.height;
		this.cubesLeft = this.canvasWidth/2 - navContainer.width;
		this.cubesTop = ((canvasContainer.height/2) - (displayCard.height/2)) + ((displayCard.height - navContainer.height)/2);
		console.log('cubes left', this.cubesLeft);
		...
		*/
		...
		const cardWidth = document.querySelector('#notification-card .display-card').getBoundingClientRect().width
		const navListWidth = document.querySelector('#notification-card .nav-container').getBoundingClientRect().width
		...
		if (this.ctx) {
			...
			this.ctx.canvas.height = this.canvasHeight;
			console.log('b');
			console.log(this.cubesLeft, this.cubesTop);
			// debugger;
	...
	methods: {
		...
		hamburgerMenuClick() {
			...
			if (this.cardActionState == 'opened' || this.cardActionState == 'opening') {
				...
				*/
				if (this.$refs['display-card'].style.transform = 'translateX(0%) translateY(-50%)') {
					...
					this.$refs['display-card'].style.transform = 'translateX(0%) translateY(-50%)';
				}
			}
		},
		...
		async translateSquares(timeStamp) {
			console.log('is it translating squares');
			const that = this;
			let isAnimationCompleted = true;
			let isMovementCompleted = true;
			if (this.ctx) {
				// clear the canvas
				await this.ctx.translate(-this.cubesLeft, -this.cubesTop);
				await this.ctx.clearRect(0, 0, that.canvasWidth, that.canvasHeight);
				await this.ctx.translate(this.cubesLeft, this.cubesTop);
				for (var row = 0; row < this.rows; row++) {
					for (let column = 0; column < this.columns; column ++) {
						let square = this.squares[row][column];
						square.doneAnimating = false;
						square.doneMoving = false;
						if (this.cardActionState == 'opening' || this.cardActionState == 'opened') {
							...
							if (square.currentEnterDelay > 0) {
								...
								if (square.opacity < 1) square.opacity = square.opacity + (square.opacityTick);
							...
							if (square.currentLeaveDelay > 0) {
								...
								//being moving the card back to center when the last square (bottom right) begins moving away
								if (row == 0 && column == 0) this.$refs['display-card'].style.transform = 'translateX(-50%) translateY(-50%)';
								...
								if (square.opacity > 0) square.opacity = square.opacity - (square.opacityTick)
						...
						await this.ctx.fillRect(0, 0, square.width + 1, square.height + 1);
				...
				if (this.cardActionState == 'opened' && isMovementCompleted) {
		...
		getPropertyIncrement(startValue, endValue, duration) {
			const TICK_TIME = 1000 / 240; //to achieve 60fps, 1 frame needs to render every 16ms
			const ticksToComplete = (duration / 8) / TICK_TIME;
		...
		Square: function(that, row, column, index) {
			...
			if (currentRow < 0) {
				...
				if (currentColumn <= val && currentColumn >= -val) {
					...
					this.xTransform = that.map(randomX, 0 , randomMultipler, 0, that.columns * 5) * xInvert;
					this.yTransform = that.map(randomY, 0, randomMultipler, 0, that.rows * 5) * yInvert;
					console.log('upper: ', this.xTransform, this.yTransform);
				}
			...
			if (row == 0 && column == 0) console.log('transform X Y', this.xTransform, this.yTransform);
			...
			this.enterDelay = that.map(index, 1, that.rows * that.columns, 0, 250) + (Math.random() * 200);
